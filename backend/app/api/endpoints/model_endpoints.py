...
models = {
    "sunny": {
        ...
        "model_path": os.path.join(model_dir, "sunny.json"),
        "model": None
    },
    "abhishek_amount": {
        "name": "Abhishek Amount Model",
        "features": [
        ...
        "model_path": os.path.join(model_dir, "abhishek_amount.json"),
        "model": None
    },
    "abhishek_ratio": {
        "name": "Abhishek Ratio Model",
        "features": [
        ...
        "model_path": os.path.join(model_dir, "abhishek_ratio.json"),
        "model": None
    }
}
    ...
    for model_id, model in models.items():
        try:
            ...
            if not os.path.exists(model_path):
                pattern = os.path.join(model_dir, f"{model_id}_v*.json")
                versioned_files = sorted(glob.glob(pattern), reverse=True)
                if versioned_files:
                    model_path = versioned_files[0]
                    print(f"Using versioned model: {os.path.basename(model_path)}")
            if os.path.exists(model_path):
                ...
                print(f"No model file found for {model['name']}")
...
async def retrain_model(params: RetrainingParams):
    try:
        ...
        try:
            transaction_api = os.getenv("NODE_TRANSACTION_API_URL", "http://localhost:5000/api/transactions/")
            response = requests.get(transaction_api)
            if not response.ok:
...
FIELD_METADATA = {
    "ip_address_flag": {"label": "IP Address Flag", "type": "select", "options": [{"value": 1, "label": "Low Risk (1)"}, {"value": 2, "label": "Medium Risk (2)"}, {"value": 3, "label": "High Risk (3)"}], "default": 1},
    "previous_fraudulent_activity": {"label": "Previous Fraudulent Activity", "type": "toggle", "default": False},
    "daily_transaction_count": {"label": "Daily Transaction Count", "type": "number", "min": 0, "default": 5},
    "failed_transaction_count_7d": {"label": "Failed Transactions (7d)", "type": "number", "min": 0, "default": 0},
    "transaction_distance": {"label": "Transaction Distance (km)", "type": "number", "step": 0.1, "min": 0, "default": 10},
    "account_age": {"label": "Account Age (days)", "type": "number", "min": 1, "default": 365},
    "is_weekend": {"label": "Is Weekend", "type": "toggle", "default": False},
    "is_night": {"label": "Is Night (22:00–06:00)", "type": "toggle", "default": False},
    "time_since_last_transaction": {"label": "Time Since Last Tx (min)", "type": "number", "min": 0, "default": 120},
    "distance_avg_transaction_7d": {"label": "Avg Distance (7d) km", "type": "number", "step": 0.1, "min": 0, "default": 15},
    "transaction_to_balance_ratio": {"label": "Tx to Balance Ratio", "type": "range", "min": 0, "max": 1, "step": 0.01, "default": 0.1},
    "transaction_amount": {"label": "Transaction Amount (₹)", "type": "number", "step": 0.01, "min": 0, "default": 100},
    "avg_transaction_amount_7d": {"label": "Avg Tx Amount (7d)", "type": "number", "step": 0.01, "min": 0, "default": 200},
    "avg_transaction_distance": {"label": "Avg Tx Distance (km)", "type": "number", "step": 0.1, "min": 0, "default": 20},
    "monthly_transaction_count": {"label": "Monthly Transaction Count", "type": "number", "min": 0, "default": 30},
    "transaction_location_flag": {"label": "Transaction Location Flag", "type": "toggle", "default": False},
    "suspicious_ip_flag": {"label": "Suspicious IP Flag", "type": "toggle", "default": False},
    "multiple_account_login": {"label": "Multiple Account Logins", "type": "number", "min": 1, "max": 50, "default": 1},
    "transaction_amount_ratio": {"label": "Tx Amount Ratio", "type": "range", "min": 0, "max": 5, "step": 0.1, "default": 0.5},
    "failed_transaction_rate": {"label": "Failed Tx Rate", "type": "range", "min": 0, "max": 1, "step": 0.01, "default": 0.05},
    "transaction_distance_ratio": {"label": "Tx Distance Ratio", "type": "range", "min": 0, "max": 5, "step": 0.1, "default": 0.8},
    "transaction_count_ratio": {"label": "Tx Count Ratio", "type": "range", "min": 0, "max": 5, "step": 0.1, "default": 0.6},
}
        ...
        for model_id, model_info in models.items():
            ...
            for filename in os.listdir(model_dir):
                if filename.startswith(model_id + "_") and filename.endswith(".json"):
                    version = filename[len(model_id)+1:-5]
                    versioned_models.append({
                        "version": version,
                        "path": os.path.join(model_dir, filename)
                    })
            
            # Build form_fields: ordered list of form field names this model uses
            form_fields = []
